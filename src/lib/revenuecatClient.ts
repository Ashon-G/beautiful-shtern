...
/**
 * Get the monthly package from the current offering (Basic tier)
 *
 * @returns RevenueCatResult containing the monthly package or null
 */
export const getMonthlyPackage = async (): Promise<
  RevenueCatResult<PurchasesPackage | null>
> => {
  return getPackage('$rc_monthly');
};

/**
 * Check if user has premium access (any paid tier)
 *
 * @returns RevenueCatResult<boolean> describing premium access state or failure
 */
export const hasPremiumAccess = async (): Promise<
  RevenueCatResult<boolean>
> => {
 ...
 *
 * @returns RevenueCatResult containing all packages or a failure reason
 */
export const getAllPackages = async (): Promise<
  RevenueCatResult<PurchasesPackage[]>
> => {
  const offeringsResult = await getOfferings();
